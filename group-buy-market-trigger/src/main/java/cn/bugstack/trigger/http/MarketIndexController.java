...
public class MarketIndexController implements IMarketIndexService {
    ...
    public Response<GoodsMarketResponseDTO> queryGroupBuyMarketConfig(@RequestBody GoodsMarketRequestDTO requestDTO) {
        try{
            ...
            // 2. 查询拼团组队
            List<UserGroupBuyOrderDetailEntity> userGroupBuyOrderDetailEntities = indexGroupBuyMarketService.queryInProgressUserGroupBuyOrderDetailList(activityId, requestDTO.getUserId(), groupBuyActivityDiscountVO.getTakeLimitCount(), groupBuyActivityDiscountVO.getTarget());
